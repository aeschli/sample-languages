    ...
    RuleFactory.getCompiledRuleId = function (desc, helper, repository) {
        if (!desc.id) {
            helper.registerRule(function (id) {
                ...
                if (!desc.begin) {
					var repo = desc.repository;
					if (repo) {
						for (var v in repository) {
							repo[v] = repository[v];
						}
					} else {
						repo = repository;
                    ...
                    return new IncludeOnlyRule(desc.id, desc.name, desc.contentName, RuleFactory._compilePatterns(desc.patterns, helper, repo));
                }
